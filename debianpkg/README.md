...
1. **Compile the Program**: First, compile the program using the following instructions:  
   ...
   5. Run the created whatly executable (if the compilation finishes without errors, close the program if it works correctly to proceed to the next steps)
...
3. **Move the Executable**: Move the "whatly" file located in the "src" folder to the "data" folder.  
...
5. **New Release**: Run the following command to set the necessary parameters for the package and its version:  
   ...
   ```bash
   dch --create -D stable --package "whatly" --newversion=4.16.0 "New release."
   ```
...
7. **Compile the Debian Package**: Execute the following command to start compiling the Debian package. If you encounter errors, please report the issue and verify that all steps were completed correctly:  
   ...
   After completing these steps, you will find the `.deb` package in the folder where you cloned the repository. By default, the folder should be named `whatly`.
